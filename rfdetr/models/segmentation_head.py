...
@torch.jit.script
def calculate_uncertainty_jit(logits: torch.Tensor) -> torch.Tensor:
    """
    Calculate uncertainty for logits.
    This is a placeholder implementation that should be replaced with the actual uncertainty function.
    """
    return -torch.abs(logits)

@torch.jit.script
def point_sample_jit(input: torch.Tensor, point_coords: torch.Tensor, align_corners: bool = True, mode: str = "bilinear") -> torch.Tensor:
    """
    A wrapper around :function:`torch.nn.functional.grid_sample` to support 3D point_coords tensors.
    Unlike :function:`torch.nn.functional.grid_sample` it assumes `point_coords` to lie inside
    [0, 1] x [0, 1] square.
...
@torch.jit.script
def get_uncertain_point_coords_with_randomness_jit(
    coarse_logits: torch.Tensor, num_points: int, oversample_ratio: int = 3, importance_sample_ratio: float = 0.75
) -> torch.Tensor:
    """
    Sample points in [0, 1] x [0, 1] coordinate space based on their uncertainty. The unceratinties
        are calculated for each point using 'uncertainty_func' function that takes point's logit
    ...
    point_coords = torch.rand(num_boxes, num_sampled, 2, device=device, dtype=coarse_logits.dtype)
    point_logits = point_sample_jit(coarse_logits, point_coords, align_corners=False, mode="bilinear")
    # It is crucial to calculate uncertainty based on the sampled prediction value for the points.
    # Calculating uncertainties of the coarse predictions first and sampling them for points leads
    # to incorrect results.
    # To illustrate this: assume uncertainty_func(logits)=-abs(logits), a sampled point between
    ...
    # However, if we calculate uncertainties for the coarse predictions first,
    # both will have -1 uncertainty, and the sampled point will get -1 uncertainty.
    point_uncertainties = calculate_uncertainty_jit(point_logits)
    num_uncertain_points = int(importance_sample_ratio * num_points)
    num_random_points = num_points - num_uncertain_points
    idx = torch.topk(point_uncertainties[:, 0, :], k=num_uncertain_points, dim=1)[1]
    shift = num_sampled * torch.arange(num_boxes, dtype=torch.long, device=device)
    idx += shift[:, None]
    point_coords = point_coords.view(-1, 2)[idx.view(-1), :].view(
        num_boxes, num_uncertain_points, 2
    )
    if num_random_points > 0:
        point_coords = torch.cat(
            [
                point_coords,
                torch.rand(num_boxes, num_random_points, 2, device=device, dtype=coarse_logits.dtype),
            ],
            dim=1,
        )
    return point_coords